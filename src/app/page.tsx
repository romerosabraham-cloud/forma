...
import { Heart, Moon, Flame, Clock, Brain, Check, Lock, User, Zap, TrendingUp, ChevronRight, RefreshCw, X, Footprints, Shield } from "lucide-react";
  ...
  return (
    <div style={{position:"relative",width:size,height:size}}>
      <svg width={size} height={size} style={{transform:"rotate(-90deg)"}}>
        ...
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={stroke} strokeDasharray={`${dash} ${circ}`} strokeLinecap="round" style={{transition:"stroke-dasharray 1.2s ease"}}/>
      </svg>
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"14px 16px",flex:1}}>
      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:10}}>
        <div style={{width:28,height:28,borderRadius:8,background:color+"22",display:"flex",alignItems:"center",justifyContent:"center"}}><Icon size={13} color={color}/></div>
        <span style={{fontSize:10,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1}}>{label}</span>
      </div>
      <div style={{fontFamily:"monospace",fontSize:21,fontWeight:600,color:C.text}}>{value}<span style={{fontSize:12,color:C.muted}}> {unit}</span></div>
...
const TTip = ({active,payload,label,unit}:any) => {
  ...
  return <div style={{background:C.card2,border:`1px solid ${C.border2}`,borderRadius:8,padding:"8px 12px"}}><div style={{fontSize:13,color:C.text}}>{payload[0].value.toLocaleString("es")} <span style={{color:C.muted}}>{unit}</span></div><div style={{fontSize:11,color:C.muted}}>{label}</div></div>;
};

// ─── ONBOARDING ───────────────────────────────────────────────
function GoalScreen({ onSelect }: any) {
  return (
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {GOALS.map(g=>(
          <button key={g.id} onClick={()=>onSelect(g.id)}
            style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:"20px 18px",display:"flex",alignItems:"center",gap:16,textAlign:"left",cursor:"pointer",width:"100%",color:C.text}}
...
function MetricsScreen({ metrics, onChange, onNext }: any) {
  ...
  const inp:any = {width:"100%",background:C.card2,border:`1px solid ${C.border2}`,borderRadius:13,padding:"14px 16px",color:C.text,fontSize:16,outline:"none"};
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"60px 22px 40px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:15}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div>
            <label style={{display:"block",fontSize:11,color:C.muted,marginBottom:6,textTransform:"uppercase" as const,letterSpacing:1}}>Sexo</label>
        ...
        {bmi&&(
          <div style={{background:"linear-gradient(135deg,#0B180B,#0A150A)",border:`1px solid ${C.accent}30`,borderRadius:14,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{fontSize:12,color:parseFloat(bmi)<25?C.accent:C.gold,textAlign:"right"}}>
              <div>{parseFloat(bmi)<18.5?"Bajo peso":parseFloat(bmi)<25?"Normal ✓":parseFloat(bmi)<30?"Sobrepeso":"Obesidad"}</div>
              <div style={{color:C.muted}}>Rango: 18.5–24.9</div>
            </div>
          </div>
        )}
      </div>
      <button onClick={onNext} style={{width:"100%",background:C.accent,color:"#000",border:"none",borderRadius:18,padding:"19px",fontSize:16,fontWeight:800,marginTop:28,cursor:"pointer"}}>Continuar →</button>
    </div>
  );
}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          ...
          <div style={{fontSize:26,fontWeight:900,letterSpacing:1,marginTop:2,color:C.text}}>Meta: <span style={{color:goalObj.color}}>{goalObj.title}</span></div>
        </div>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:22,padding:20,display:"flex",alignItems:"center",gap:18}}>
        <Ring pct={steps/10000} color={C.accent} size={94} stroke={9}>
          <div style={{textAlign:"center"}}><div style={{fontSize:13,fontWeight:600,color:C.accent}}>{Math.round(steps/100)}%</div></div>
        </Ring>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:15,marginBottom:14,color:C.text}}>Progreso diario</div>
          {[{l:"Pasos",v:steps.toLocaleString("es"),g:"10.000",pct:steps/10000,c:C.accent},{l:"Calorías",v:calories,g:"500 kcal",pct:calories/500,c:C.red},{l:"Min. activos",v:activeMin,g:"45",pct:activeMin/45,c:C.blue}].map(it=>(
            <div key={it.l} style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:12,color:C.muted}}>{it.l}</span><span style={{fontSize:12,color:C.text}}>{it.v} <span style={{color:C.muted}}>/ {it.g}</span></span></div>
              <div style={{height:4,background:it.c+"25",borderRadius:2}}><div style={{height:"100%",width:`${Math.min(it.pct*100,100)}%`,background:it.c,borderRadius:2,transition:"width 1.2s ease"}}/></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{background:"linear-gradient(135deg,#0C1A04,#0A1502)",border:`1px solid ${C.accent}35`,borderRadius:22,padding:20}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><Brain size={17} color={C.accent}/><span style={{fontSize:14,fontWeight:700,color:C.accent}}>Coach IA · Análisis de hoy</span></div>
          <button onClick={onRefreshAI} style={{background:"transparent",border:"none",padding:4,cursor:"pointer",display:"flex"}}><RefreshCw size={14} color={C.muted}/></button>
        </div>
        {aiLoading
          ? <div style={{display:"flex",gap:8,alignItems:"center"}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:C.accent,opacity:.6}}/>)}<span style={{color:C.muted,fontSize:13,marginLeft:4}}>Analizando...</span></div>
          : <div style={{fontSize:14,color:C.text,lineHeight:1.65}}>{aiInsight||"Pulsa ↺ para tu análisis personalizado"}</div>}
      </div>
    </div>
  );
}
...
function ActivityTab() {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{fontSize:34,fontWeight:900,color:C.text,letterSpacing:1}}>ESTA <span style={{color:C.accent}}>SEMANA</span></div>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:22,padding:20}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <div><div style={{fontSize:11,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1}}>Pasos diarios</div><div style={{fontSize:26,fontWeight:700,color:C.accent,marginTop:4}}>{avgSteps.toLocaleString("es")} <span style={{fontSize:13,color:C.muted}}>media/día</span></div></div>
        ...
        <ResponsiveContainer width="100%" height={140}>
          <AreaChart data={W_STEPS} margin={{top:5,right:0,bottom:0,left:0}}>
            <defs><linearGradient id="sg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.accent} stopOpacity={.35}/><stop offset="100%" stopColor={C.accent} stopOpacity={0}/></linearGradient></defs>
            <XAxis dataKey="d" tick={{fill:C.muted,fontSize:12}} axisLine={false} tickLine={false}/>
            <YAxis hide/><Tooltip content={<TTip unit="pasos"/>}/>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:22,padding:20}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <div><div style={{fontSize:11,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1}}>Calorías quemadas</div><div style={{fontSize:26,fontWeight:700,color:C.red,marginTop:4}}>{totCal.toLocaleString("es")} <span style={{fontSize:13,color:C.muted}}>esta semana</span></div></div>
        ...
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={W_CAL} margin={{top:5,right:0,bottom:0,left:0}} barSize={22}>
            <XAxis dataKey="d" tick={{fill:C.muted,fontSize:12}} axisLine={false} tickLine={false}/>
            <YAxis hide/><Tooltip content={<TTip unit="kcal"/>}/>
...
function BodyTab({ metrics }: any) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{fontSize:34,fontWeight:900,color:C.text,letterSpacing:1}}>COMPOSICIÓN <span style={{color:C.accent}}>CORPORAL</span></div>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:22,padding:20}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <div><div style={{fontSize:11,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1}}>Tendencia de peso</div><div style={{fontSize:30,fontWeight:700,color:C.text,marginTop:4}}>{cur} <span style={{fontSize:14,color:C.muted}}>kg</span></div></div>
          <div style={{background:C.accent+"18",border:`1px solid ${C.accent}30`,borderRadius:12,padding:"10px 16px",textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:C.accent}}>-{(start-cur).toFixed(1)}</div><div style={{fontSize:11,color:C.muted}}>kg · 8 sem.</div></div>
        </div>
        <ResponsiveContainer width="100%" height={155}>
          <LineChart data={W_WEIGHT} margin={{top:5,right:0,bottom:0,left:0}}>
            ...
            <YAxis hide domain={[78,84]}/><Tooltip content={<TTip unit="kg"/>}/>
            <Line type="monotone" dataKey="v" stroke={C.accent} strokeWidth={2.5} dot={{fill:C.accent,r:3}} activeDot={{r:5,fill:C.accent}}/>
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:18}}><div style={{fontSize:11,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1,marginBottom:8}}>IMC</div><div style={{fontSize:36,fontWeight:800,color:parseFloat(bmi)<25?C.accent:C.gold}}>{bmi}</div><div style={{fontSize:12,marginTop:5,color:parseFloat(bmi)<25?C.accent:C.gold}}>{parseFloat(bmi)<25?"Normal ✓":"Sobrepeso"}</div></div>
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:18}}><div style={{fontSize:11,color:C.muted,textTransform:"uppercase" as const,letterSpacing:1,marginBottom:8}}>% Grasa Est.</div><div style={{fontSize:36,fontWeight:800,color:C.blue}}>18.4%</div><div style={{fontSize:12,marginTop:5,color:C.blue}}>Saludable ✓</div></div>
      </div>
    </div>
  );
}
...
function CoachTab({ metrics, goal }: any) {
  const [msgs, setMsgs] = useState([{role:"assistant",t:"¡Hola! Soy tu Coach IA. ¿En qué puedo ayudarte hoy?"}]);
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({system:`Coach personal. Objetivo=${gObj.title}, peso=${metrics.weight}kg. Responde en español, máx 100 palabras.`,messages:newMsgs.map(m=>({role:m.role,content:m.t})),max_tokens:300})});
      const d = await res.json();
      setMsgs([...newMsgs,{role:"assistant",t:d.content?.[0]?.text||"¡Sigue adelante!"}]);
    } catch { setMsgs([...newMsgs,{role:"assistant",t:"¡Sigue con tu buen trabajo!"}]); }
    setLoading(false);
  };

  return (
    <div style={{display:"flex",flexDirection:"column",gap:12,height:"calc(100vh - 200px)"}}>
      <div style={{fontSize:34,fontWeight:900,color:C.text,letterSpacing:1}}>COACH <span style={{color:C.accent}}>IA</span></div>
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12,paddingBottom:8}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            <div style={{maxWidth:"78%",background:m.role==="user"?C.accent:C.card,color:m.role==="user"?"#000":C.text,borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px",padding:"12px 16px",fontSize:14,lineHeight:1.55}}>{m.t}</div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",gap:6,padding:"12px 16px",background:C.card,borderRadius:"18px 18px 18px 4px",width:"fit-content"}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:C.muted,opacity:.7}}/>)}</div>}
      </div>
      <div style={{display:"flex",gap:10,background:C.card,border:`1px solid ${C.border2}`,borderRadius:18,padding:"8px 8px 8px 16px",alignItems:"center"}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Pregunta a tu coach..." style={{flex:1,background:"transparent",color:C.text,fontSize:14,border:"none",outline:"none"}}/>
        <button onClick={send} style={{background:C.accent,border:"none",borderRadius:13,width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
    ...
    <div style={{position:"fixed",inset:0,background:"#00000088",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"28px 28px 0 0",padding:"32px 24px 52px",width:"100%",maxWidth:480,border:`1px solid ${C.border2}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          <button onClick={onClose} style={{background:C.card2,border:`1px solid ${C.border}`,width:34,height:34,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><X size={15} color={C.muted}/></button>
        </div>
        <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Desbloquea tu potencial completo</div>
        <div style={{display:"flex",flexDirection:"column",gap:11,marginBottom:24}}>
          {[["🧠","Coach IA sin límites"],["📊","Analíticas avanzadas"],["🏋️","Planes adaptativos"],["🥗","Nutrición personalizada"],["⌚","Wearables"],["📈","Historial ilimitado"]].map(([e,t])=>(
            <div key={t as string} style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:19}}>{e}</span><span style={{fontSize:14,color:C.text,flex:1}}>{t}</span><Check size={14} color={C.accent}/></div>
          ))}
        </div>
        ...
        <button onClick={onUpgrade} style={{width:"100%",background:C.gold,color:"#000",border:"none",borderRadius:18,padding:"19px",fontSize:16,fontWeight:800,cursor:"pointer"}}>⭐ Probar 7 días GRATIS</button>
      </div>
    </div>
  );
}
...
export default function FormaApp() {
  ...
  const fetchAI = async () => {
    ...
    try {
      ...
      const res=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:`Coach. Insight motivador en español (2-3 frases). Objetivo=${gLabel}, pasos=${steps}, IMC=${bmi}, peso=${metrics.weight}kg.`}],max_tokens:200})});
      const d=await res.json();
      setAiInsight(d.content?.[0]?.text||null);
    } catch { setAiInsight(`Con ${steps.toLocaleString("es")} pasos llevas ${Math.round(steps*.04)} kcal quemadas hoy. ¡Sigue así!`); }
    setAiLoading(false);
  };

  // Mostrar loading mientras Clerk carga
  if (!isLoaded) {
    return <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:36,fontWeight:900,color:C.accent,letterSpacing:8}}>FORMA</div></div>;
  }

  // Mostrar landing si no está autenticado
  if (!isSignedIn) {
    return <LandingPage />;
  }

  // Onboarding
  if (screen==="goal") return <GoalScreen onSelect={(g:string)=>{setGoal(g);setScreen("metrics");}}/>;
  if (screen==="metrics") return <MetricsScreen metrics={metrics} onChange={setMetrics} onNext={()=>{setScreen("app");setTimeout(fetchAI,900);}}/>;

  const TABS=[{id:"overview",l:"Inicio"},{id:"activity",l:"Actividad"},{id:"body",l:"Cuerpo"},{id:"coach",l:"Coach IA",pro:true}];
